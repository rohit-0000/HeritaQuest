...
const LocationQuizResult = () => {
  const { id } = useParams();
  const quiz = useSelector((state) =>
    state.heritaQuest.LocationQuizHistory.find(
  ...
  return (
    <div className="w-screen h-screen flex flex-col justify- items-center bg-white pt-20 px-5">
      <h1 className="text-3xl font-bold pb-10">MCQ Quiz Result</h1>
      ...
      <div className="flex gap-10 py-10">
        <NavLink to={`/locationQuizReview/${quiz?.id}`} className="px-8 py-2 bg-blue-500 rounded active:scale-95 transition-transform duration-75 cursor-pointer text-white text-xl">Review</NavLink>
        <NavLink to={`/locationQuiz/${quiz?.id}`} className="px-8 py-2 bg-green-500 rounded active:scale-95 transition-transform duration-75 cursor-pointer text-white text-xl">Re-Attempt</NavLink>
      </div>
...
export default LocationQuizResult;
